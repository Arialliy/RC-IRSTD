...
date: "2026-07-14"
language: "zh-CN"
source_document: "02_RC-IRSTD_方案_代码_步骤(1).md"
target_venue: "AAAI-27 Main Technical Track"
---

# RC-IRSTD：AAAI-27 投稿可行性评审与修改建议

> 分析对象：`02_RC-IRSTD_方案_代码_步骤(1).md`
...
AAAI-27 主会官方时间表如下，所有截止时间均为 AoE，即 UTC-12：

| 事项 | 官方截止时间 | 台北时间 UTC+8 |
|---|---:|---:|
...
- **7 页主内容**；
- 最多 9 页，超过第 7 页的页面只能放参考文献；
- 双盲评审；
- 两阶段评审；
- Phase 1 负面评价足够明显时会直接拒稿，没有作者回复机会；
- 补充材料不保证被评审阅读；
- 必须提交 reproducibility checklist。
...
当前最高阈值若仅到 `0.99999`，仍可能低于最大预测分数，从而人为制造“无可行阈值”。
...
更稳妥的方式是在 logit 空间加入高于最大 score 的哨兵阈值。
...
AAAI-27 只有 7 页主内容，不能把完整工程方案全部搬入主文。
...
最可能导致 Phase 1 拒稿的原因依次是：

1. 手工统计加 MLP 回归阈值显得启发式；
2. support 与 oracle query 未分离，causal claim 不成立；
3. oracle/reject 定义存在形式矛盾；
...
1. [AAAI-27 Conference Homepage](https://aaai.org/conference/aaai/aaai-27/)
2. [AAAI-27 Main Technical Track Call](https://aaai.org/conference/aaai/aaai-27/main-technical-track-call/)
3. [AAAI-27 Review Process](https://aaai.org/conference/aaai/aaai-27/review-process/)
4. [AAAI-27 Areas and Topics](https://aaai.org/conference/aaai/aaai-27/areas-and-topics/)
