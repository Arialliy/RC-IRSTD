...
if ((require_clean_release)); then
  ...
  if ! git tag --points-at HEAD | grep -Fxq 'aaai27-rc-irstd-v5-rc1'; then
    echo "Clean-release preflight requires tag aaai27-rc-irstd-v5-rc1 at HEAD" >&2
    exit 1
  fi
  if [[ ! -s outputs/release/RC-IRSTD_v5_rc1.zip ]] \
    || [[ ! -s outputs/release/RC-IRSTD_v5_rc1.zip.sha256 ]]; then
    echo "Clean-release preflight requires the source archive and SHA file" >&2
    exit 1
  fi
  sha256sum -c outputs/release/RC-IRSTD_v5_rc1.zip.sha256 \
